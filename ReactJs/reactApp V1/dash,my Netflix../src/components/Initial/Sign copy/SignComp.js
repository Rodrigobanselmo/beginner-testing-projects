import {Background, Error, Text,TextButton,InputConfirm,TextForgotten, TextSmall,HeroBg,DivIcon,IconWarn,IconCheck,IconError,Google,VideoBg, LinkButton, Input, Submit,SocialLogo,Container } from './SignCss';
...
Sign.Errors =  function Errors(props) {
    return(
      props.error && <Error data-testid="error">{props.error}</Error>
    )
}
...
Sign.GoogleButton =  function GoogleButton() {
    return(
      <Google type="submit" google={true}>
...
Sign.DashOrDash = function DashOrDash() {
    return (
      <div style={{display:'flex', alignItems:'center',justifyContent:'space-between',marginTop:-10,height:20}}>
        <div style={{width:'43%',height:1,backgroundColor:'grey'}}/>
        <p style={{color:'grey',marginTop:10}}>ou</p>
        <div style={{width:'43%',height:1,backgroundColor:'grey'}}/>
      </div>
    )
}
  
Sign.Cadastrar = function Cadastrar({text,fade,change,onChangeAuth}) {
    return(
      <Text fade={fade}>
        {text[change?2:4]} <TextButton onClick={onChangeAuth}>{text[change?3:5]}</TextButton>
      </Text>
    )
}
