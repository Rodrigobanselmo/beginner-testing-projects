...
const Principal = ({CheckListData=[],_key=1,navigation,dispatch}) => {

    const [secondary, setSecondary] = useState(false);
    const [activeIndex, setactiveIndex] = useState(0)
    const [previewIndex, setPreviewIndex] = useState(0)

    const data = CheckListData.data[_key-1].questions.filter(i=>!(i?.parent&&i.parent))
    const group = CheckListData.data[_key-1].group
    const groupId = CheckListData.data[_key-1].id
        ...
        <FlingGestureHandler key='LEFT' direction={Directions.LEFT} onHandlerStateChange={ev=>{
            ...
            <FlingGestureHandler key='RIGHT' direction={Directions.RIGHT} onHandlerStateChange={ev=>{
                                        ...
                                        <Animated.View  style={{position:'absolute',backfaceVisibility:'hidden', transform: [{translateY},{rotateY:animatedFlipBack},{translateX},{scale}],opacity,zIndex:data.length*2-index*2+(isFront?0:1),elevation:data.length*2-index*2+(isFront?0:1), left:(windowWidth-CARD_WIDTH)/2, top:0}}>
                                            <CardView style={{height:CARD_HEIGHT, width:CARD_WIDTH,}} >
                                                <ScrollView contentContainerStyle={{ flexGrow: 1}} showsVerticalScrollIndicator={false} style={{width:'100%'}}>
                                                    <Observation value={value} setValue={setValue} setIsFront={setIsFront} onAnimatedFlip={onAnimatedFlip} group={'group'} item={item}/>
                                                </ScrollView>
                                            </CardView>
                                        </Animated.View>
                ...
                /*           onPress={onAdd} */
                        ...
                        disabledButton={true}
                        />
                        <TouchableHighlight activeOpacity={0.5} underlayColor={themeContext.background.hover} style={{zIndex:1000,padding:9,borderRadius:30}} onLongPress={() => {setactiveSlide(data.length-1)}} onPress={() => {if (activeIndex < data.length-1) setactiveSlide(activeIndex+1)}}>
