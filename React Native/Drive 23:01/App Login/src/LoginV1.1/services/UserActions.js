      ...
      .get().then((docData) => {
        if (docData.exists) {
          ...
          if (docData.data().emailVerified) {
            onModalVisible(true,'loaderScreen',()=>navigationActions('DrawerStack','TabStack'))
          } else {
            onModalVisible(true,'loaderScreen',()=>navigationActions('DrawerStack','VerificationStack'))
          }
        ...
        if (docData.exists) {
          if (docData.data().providerId.includes("google.com")) {
            ...
            usersDocRef.update({...userUpdate}).then(()=>{
              ...
              if ([...userLogin.providerId].length <= 1) onModalVisible(true,'loaderScreen',()=>navigationActions('DrawerStack','TabStack'))
            }).catch(erro=>{
  ...
  return async (dispatch) => {
    await firestore().collection('user').doc(data.uid).update({...data.changeData}).then(()=>{
      ...
      if (data?.action && data.action == 'navigate') {
        onModalVisible(true,'loaderScreen2',()=>navigationActions('DrawerStack','TabStack'),true)
