...
import {infoNet,updateUserEmail,ChangeUserData,EmailSignAfter,GoogleSignAfter} from '../../services/FirebaseAuth';
...
export default function App({navigation}) {
  ...
  const [dispatchUserGoogle,__,___,dispatchUserDataChange] = useDispatchActions(onModalVisible)
  ...
  function onChangeType(types,_email) {
    ...
    if (types == 'add_pass') {
      ...
      infos = {
        ...
        placeholder:['e-mail'],
        preLoaded:[email],
        onFunc: (onAnimationModal,t1,t2,t3,valid,onClose)=>{updateUserEmail(t1,null,false,setEmail,onAnimationModal,onSetModalVisible,onModalVisible,setaskPass,onClose,dispatchUserDataChange)}
      }
      ...
      infos = {
        ...
        typeInput:['pass'],
        placeholder:['senha'],
        onFunc: (onAnimationModal,t1,t2,t3,valid,onClose)=>{updateUserEmail(_email,t1,true,setEmail,onAnimationModal,onSetModalVisible,onModalVisible,setaskPass,onClose,dispatchUserDataChange)}
      }
    }
  ...
  const onSetModalVisible = (activation,types) => {
  ...
  function onChangeEmail() {
    if (user.providerId.includes("password")) {
      infoNet(
  ...
  function onGoogleLink() {
    infoNet(
      ()=>GoogleSignAfter(onModalVisible,user.providerId.includes("google.com")?true:false,user.providerId,dispatchUserGoogle,dispatchUserDataChange),
      ()=>onModalVisible(true,'info','Você não está conectado à internet. Por favor, tente novamente mais tarde.','Erro no Login',0)
    )
  }
    ...
    <SafeAreaView>
      ...
      <ScrollView>
        <View style={{marginHorizontal:20,marginTop:20}}>
          ...
          <Dados onPress={onChangeEmail} type={'e-mail'} value={user?.email ? user.email : '-'} />
