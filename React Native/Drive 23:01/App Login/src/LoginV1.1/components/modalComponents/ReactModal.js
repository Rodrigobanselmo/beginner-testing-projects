...
const useReactModal = (setCallBack) => {
  ...
  const onCloseLate = () => {
    if (date+2500<(new Date()-0)) {
      setModalVisible(true)
      setTypeModal('info')
      setText(`Tempo exedido,verifique sua conexão com a internet e tente novamnete mais tarde.`)
      setCallBack(Math.random())
    }
  }

  const onModalVisible = (visible,type,text,title,load) => {

      setTimeout(() => {
        setModalVisible(visible)
        Tipo()
        setCallBack(Math.random())
      }, load?load: visible? type=='info'? 1000:0 :1000);
    ...
    function Tipo() {
      if (type) {
        ...
        } else if (type ==  'loaderScreen' || type == 'loaderScreen2') {
          setText({onFunc:text})
        } else if (type ==  'option') {
          setText({
  ...
  const ModalLoadScreen = ({}) => {
    return(
      <View style={{backgroundColor: typeModal =='loaderScreen2' ? 'rgba(10, 10, 10, 0.6)' :'#ffff',justifyContent:`center`,alignItems:"center",flex:1}}>
          <LottieView onAnimationFinish={()=>{text.onFunc()}} style={{height:220,width:220}} source={typeModal =='loaderScreen2' ? require('../../../animations/18479-success.json') : require('../../../animations/6717-loading-passed-state.json')} autoPlay loop={false}/>
    </View>
    )
  }
