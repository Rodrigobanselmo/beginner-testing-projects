    ...
    useEffect(() => { 
      ...
      if(!seguir[3][0] && !seguir[3][1] && userReaload[0] != undefined) { //se esta vindo do server e nada pendente
          ...
          if (pendingMessages.length>0) {
            ...
            setpendingMessages(pendingMessages.filter(i=>i.key!=pendingMessages[0].key))
          }
    ...
    AddUserIdQulaquer = async (connect) => { 
      ...
      if (connect) {
        ...
        .collection('chats').doc('chatTest').collection('menssagens').doc(UUID).set(
          {name:connect.name,item:connect.item,dateSent:connect.dateSent,id:connect.id, date: DATE,time:`${TIME}`,}
...
{/*                 <Text>User ID: {item.key}</Text> */}
                ...
                <Text>{item?.offline ? 'false' : item.date < chat.lastUpdate ? 'true': 'false'}</Text>
              </View>
            )}
          />
