            ...
            <TouchableHighlight_Box underlayColor={colors.UnderBox} onPress={onPress ? ()=>onPress(index) : null}>
                ...
                <TouchableOpacity onPress={setIcon} >
                    <View style={styles.Icons_View}  backgroundColor={data.imageBack} >
                            {data.image !=='font'?
                            <FontAwesome5 name={data.image} size={data.image == 'university' ? 48 : 42} color='#fff' />
                            :
                            <Text style={{color: '#fff', fontSize: 49 }}>{data.title == '' ? '' : data.title.substring(1,-2).toUpperCase()}</Text>
                            }
                    </View>
                </TouchableOpacity>
                    <View_Components flex={1} alingItens='center' justifyContent='center' >
                        <View_Components flex={1} alingItens='center' justifyContent='center' >     
                            ...
                            <Text_SubTitle>{`Você tem ${data.list ? data.list.length : 0} tarefas pendentes:`}</Text_SubTitle>
                        </View_Components>
                    </View_Components>
                    <Entypo style={{marginRight:-20}} name="dots-three-vertical" color='#000' size={25} backgroundColor="transparent" > </Entypo>
                </View_Components>
                        </>
