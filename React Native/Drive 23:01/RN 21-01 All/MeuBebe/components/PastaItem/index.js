...
export default ({ data, index, onPress, onDelete, onArquivar, onDelete2, onEdit, colors }) => {
    ...
    return (
        ...
        <View flex={1} style={[styles.square]} marginHorizontal={0}>
            ...
            <TouchableOpacity onLongPress={openRight} delayLongPress={500} underlayColor={colors.UnderBox} onPress={onPress ? ()=>onPress(index) : null} style={{padding:15,borderRadius:10}}>
                <>
                ...
                <View flexDirection='row' justifyContent={'space-around'}>
                    ...
                    <View flex={1} alingItens='center' justifyContent='center' >
                        <View flex={1} alingItens='center' justifyContent='center' >     
                            <Text style={{fontSize: 22, color: '#262626', marginBottom: 0,fontWeight: 'bold', paddingLeft:17}} >{data.title}</Text>
                            <Text style={{marginTop:1,marginBottom:3,paddingLeft:17,fontSize: 14,color: 'grey'}}>{`Você tem ${data.list ? data.list.length : 0} tarefas pendentes:`}</Text>


                            { data.list &&  data.list.length > 0 ?
                                <View flexDirection='row' flexWrap='wrap'  backgroundColor='transparent' style={{marginTop:4,marginBottom:3, paddingLeft:17}}> 
                                    {data.list.map((d,k)=>
                                    d.progress === 1 || d.finalizada === false || d.pausada === true ? null :
                                        <View backgroundColor={d.backgroundColor} key={k} style={{height: 10, width: 10, borderRadius: 5, marginRight: 15}}/>
