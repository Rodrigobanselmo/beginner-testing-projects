...
export default ({ data, index, naofinalizada,pausar,concluirTarefa, colors }) => {
        ...
        const renderRightAction = (text, color, x, progress, colors, func) => {
            ...
            return (
              <Animated.View style={{transform: [{ translateX:0 }], width: x==192 ? 80 : 60, height: 77 }}>
                <TouchableOpacity
                  onPress={()=>func()}
                  style={[styles.rightAction, { backgroundColor: colors ,borderTopRightRadius:x==64 ? 12 : 0, borderBottomRightRadius:x==64 ? 12 : 0, paddingLeft: x==192 ? 16 : 0}]}>
                    <Ionicons  name={text} size={22} color={color} />
                </TouchableOpacity>
              </Animated.View>
            );
          };
          
          const getRightContent2 = progress => {
              return(
            <View style={{flexDirection: 'row', paddingRight:10, marginLeft:-32, justifyContent:'center', alignItems:'center'}}>
              {renderRightAction('play', colors.carbon, 192, progress, colors.carbonLight, openRightpause)}
              {renderRightAction('ios-close-sharp', colors.not, 128, progress, colors.notLight,openRightnotdone)}
              {renderRightAction('ios-checkmark-sharp', colors.green, 64, progress, colors.greenLight,openRight)}
            </View>
          )};
...
/*         setisOpen(true) */
     ...
     const [bom, setbom] = React.useState(0)
     const testes = () => {
        setbom(Math.random())
        console.log(bom)
    }
    ...
    return (
        ...
        onSwipeableRightOpen={testes}
        renderRightActions={bom > 0.65 ? getRightContent:getRightContent2 }
