...
export default props => {
    ...
    const initialState = { id: 0, title: '', body: '', date: today, tempoestimado: '12h', para: 'Sem data prevista', progress: 0.0, progressColor:'#2956E0', backgroundColor:['#2471A3','#154360'], ordem:100, subtarefas:[], nunSub:3, subCheck:0, finalizada:true, pausada: false}
    ...
    const onDateCancel = () => {
     ...
     const save = () => {
        ...
        if(dadosiniciais.title.trim() !='') {
            ...
            var progress1 = 0
            if (Juntar.length > 0) {
                progress1 = Math.round(itemCheck/Juntar.length*100)/100
            } 

            

            props.onSave && props.onSave({...dadosiniciais, id: dadosiniciais.id == 0 ? Math.random() : dadosiniciais.id,subtarefas:Juntar, progress:progress1, nunSub: Juntar.length > 0 ? Juntar.length : 3, subCheck:itemCheck, isStart: itemCheck > 0 ? true : false})
            setOutrosDados({...initialDados})
            setdadosiniciais({ ...initialState })
        }
    else {
