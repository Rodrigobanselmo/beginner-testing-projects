            ...
            {loading ?
              ...
              filter().length > 0 ?
                filter().map((item,indexItem)=>{
                  return (
                    <BootstrapTooltip placement="right" enterDelay={1600} enterNextDelay={1600}	TransitionProps={{ timeout: {enter:500, exit: 50} }} title={item.text} styletooltip={{transform: 'translateY(-0px)'}}>
                      <div>
                        <CardDrop
                          title={item.text}
