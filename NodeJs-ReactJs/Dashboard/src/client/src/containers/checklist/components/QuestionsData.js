...
export function QuestionsData({
  ...
  function onDuplicate() {
    ...
    var newAction = {...question.action}
    newAction = clone(newAction)
    Object.keys(question.action).map((key,index)=>{
      if (question.action[key]?.child) delete newAction[key]['child']
    })

    copyDataChecklist.data[categoryIndex].questions = [...copyDataChecklist.data[categoryIndex].questions,{...question,id:uid,text:title,action:{...newAction}}]
