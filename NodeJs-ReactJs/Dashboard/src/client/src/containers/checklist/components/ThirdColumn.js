...
export const ThirdColumn = React.memo((
  ...
  function onDuplicate() {
    ...
    questions.map((item)=>{
      let copyItem = {...item}
      copyItem = clone(copyItem)
      var newAction = {...copyItem.action}
      Object.keys(copyItem.action).map((key,index)=>{
        if (copyItem.action[key]?.child) delete newAction[key]['child']
      })
      const uid = Math.floor((1 + Math.random()) * 0x100000000000).toString(16).substring(1);
      newQuestions.push({...copyItem,id:uid,group:title})
    })
