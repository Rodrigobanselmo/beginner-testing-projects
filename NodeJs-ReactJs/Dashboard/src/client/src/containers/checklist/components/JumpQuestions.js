...
//[...suggestion().filter(i=>data.rec.includes(i.id))]
  return (
    <>
      ...
      {(provided,snapshot) => (
        <div ref={provided.innerRef} {...provided.droppableProps} style={{overflowY:'auto',height:'94%'}}>
          <AddedRiskContainer
              ...
              <>
                <>
                  {Array.isArray(jumpGroup) && jumpGroup.length >0 && jumpGroup.map((item,indexItem)=>{
                    ...
                    return (
                      <BootstrapTooltip enterNextDelay={1600} placement="right" TransitionProps={{ timeout: {enter:500, exit: 50} }} title={groups} styletooltip={{transform: 'translateY(0px)'}}>
                        <div>
                          <CardDrop
                            title={groups}
                ...
                <>
                  {Array.isArray(jumpQuestion) && jumpQuestion.length >0 && jumpQuestion.map((item,indexItem)=>{
                    ...
                    return (
                      <BootstrapTooltip enterNextDelay={1600}	placement="right" enterDelay={1600} TransitionProps={{ timeout: {enter:500, exit: 50} }} title={question.text} styletooltip={{transform: 'translateY(-0px)'}}>
                        <div>
                          <CardDrop
                            title={question.text}
