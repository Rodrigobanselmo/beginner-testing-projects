...
function getOptions(question) {
  if (question.type == 'standard') {
    ...
    Object.keys(question.action).sort().map(key=>{
...
export function QuestionColumn({
  ...
  function onChangeQuestionType(value,bool,again,func) {
    ...
    if (type == 'standard') {
      ...
      if (!keys.includes('q_1')) {
        newActions['q_1'] = {id:'q_1',text:'SIM',data:[]}
      }
      if (!keys.includes('q_2')) {
        newActions['q_2'] = {id:'q_2',text:'NÃO',data:[]}
      }
      if (!keys.includes('q_3')) {
        newActions['q_3'] = {id:'q_3',text:'N.A.',data:[]}
      }
